...
The important thing is that any errors in the input batch are likely to result in SIP output that is either unexpected or just plain wrong. So *always* verify each batch first, and fix any errors if necessary:
...
The input batch is simply a directory that contains a number of subdirectories. All of these subdirectories (except one) represents exactly one data carrier. The exception is the *scans* directory, which contains scanned cover images for each PPN. Furthermore the batch contains a *batch manifest*, which is a comma-delimited text file with basic metadata about each carrier, and a log file with details about the imaging and ripping procedure. The diagram below shows an example of a batch that contains 3 carriers (one audio CD and two CD-ROMs):
...
The *scans* directory contains one or more subdirectories, each of which represents one PPN. The name of each subdirectory corresponds to its respective PPN.
Each PPN-level subdirectory contains:

1. One or more image files in TIFF format.
2. A file *info.xml* with basic metadata (including checksums) for each TIFF image.
...
4. *title* - text string with the title of the carrier (or the publication is is part of). Not used by omSipCreator.
5. *volumeID* - text string, extracted from Primary Volume descriptor, empty if cd-audio. Not used by omSipCreator.
6. *success* - True/False flag that indicates status of *iromlab*'s imaging process.
7. *containsAudio* - True/False flag that indicates the carrier contains audio tracks (detected by cd-info)
8. *containsData* - True/False flag that indicates the carrier contains data tracks (detected by cd-info)
9. *cdExtra* - True/False flag that indicates the carrier is an 'enhanced' CD with both audio and data tracks that are located in separate sessions (detected by cd-info)
10. *mixedMode* - True/False flag that indicates the carrier is a 'mixed mode' CD with both audio and data tracks that are located in one single session (detected by cd-info)
11. *cdInteractive* - True/False flag that indicates the carrier is an interactive Philips 'cd-i' disc (detected by cd-info)

Below is a simple example of manifest file:

    jobID,PPN,volumeNo,title,volumeID,success,containsAudio,containsData,cdExtra,mixedMode,cdInteractive
    383c78fa-34a6-11e7-926c-7446a0b42b9a,18594664X,1,Marjan Berk,ELL3,True,True,True,True,False,False
    61c3e58a-34a6-11e7-98d9-7446a0b42b9a,230370241,1,Kikker is verliefd,SPELEN_MET_KIKKER,True,False,True,False,False,False
    06e80cb6-34a7-11e7-8466-7446a0b42b9a,378374036,1,Na klar!. Luister- en kijkboxen. 6 vwo,,True,True,False,False,False,False
    3cba3e5e-34a7-11e7-8bd1-7446a0b42b9a,378374036,1,Na klar!. Luister- en kijkboxen. 6 vwo,NEW,True,False,True,False,False,False
...
Each SIP is represented as a directory. Each carrier that is part of the SIP is represented as a subdirectory within that directory. Scanned cover images are stored in a *scans* directory. The SIP's root directory contains a [METS](https://www.loc.gov/mets/) file with technical, structural and bibliographic metadata. Bibliographic metadata is stored in [MODS](https://www.loc.gov/standards/mods/) format (3.4) which is embedded in a METS *mdWrap* element. Here's a simple example of a SIP that is made up of 2 carriers (which are represented as ISO 9660 images):
